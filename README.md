...
A related cluster surfaced separately: `18.9.28.1`, `18.9.28.3`, `18.9.28.4` are all named "...on domain-joined computers" but - unlike their siblings `18.9.28.5`/`.6`/`.7`, which do check it - have no actual domain-join guard in their tasks, so they apply unconditionally even on a non-domain PC. `18.9.28.1` sets `BlockUserFromShowingAccountDetailsOnSignin=1`; Microsoft's own documented behavior for this policy is that it doesn't just hide account details on the lock screen, it disables Microsoft-account-backed connected experiences entirely - OneDrive sync, Mail/Calendar (Outlook), Store sign-in. Confirmed live: this, not `2.3.1.1`, was what broke OneDrive/Outlook sign-in specifically (`NoConnectedUser` only blocks the raw "sign in with a Microsoft account" OS dialog; this one is broader and independent of it). `18.9.28.3` (`DontEnumerateConnectedUsers`) and `18.9.28.4` (`EnumerateLocalUsers`) are the same pattern, directly adjacent, and excluded alongside it rather than waiting to hit each one individually.

None of those four turned out to be the whole story either - the "blocked by group policy (0x800704ec)" dialog persisted after all of them were cleared, this time blocking Outlook specifically. The actual comprehensive cause: `18.10.41.1`, whose own name says exactly what it does - **"Block all consumer Microsoft account user authentication"** - sets `HKLM:\Software\Policies\Microsoft\MicrosoftAccount\DisableUserAuth=1`, a dedicated, system-wide kill switch for consumer MSA auth, separate from and more comprehensive than everything above. Confirmed live: clearing this (and only this, at that point) let Outlook sign in. `win11cis_rule_18_10_41_1` now defaults to `false`, Windows 11 only, same reasoning as the others.
